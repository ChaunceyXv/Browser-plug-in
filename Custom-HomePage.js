...
(function() {
    ...
    const Search = {
        ...
        render() { const c = Config.load(); this.engines = c.searchEngines; this.currentEngine = this.engines.find(e => e.name === c.defaultEngine) || this.engines[0]; const dn = Utils.truncate(this.currentEngine.name, 6); return `<style>#search-area{position:absolute;top:30%;left:50%;transform:translateX(-50%);width:80%;z-index:1}.search-logo{display:inline-block;text-align:center;margin-bottom:40px;font-size:48px;font-weight:700;color:var(--engine-color);transition:color .2s;cursor:pointer;-webkit-tap-highlight-color:transparent;user-select:none;-webkit-user-select:none}@keyframes glow-pulse{0%{box-shadow:0 0 8px 0 var(--engine-color-33),0 0 16px 0 var(--engine-color-1a)}50%{box-shadow:0 0 16px 0 var(--engine-color-33),0 0 28px 0 var(--engine-color-1a)}100%{box-shadow:0 0 8px 0 var(--engine-color-33),0 0 16px 0 var(--engine-color-1a)}}.search-box{display:flex;align-items:center;background:transparent;border:2px solid #e0e0e0;border-radius:12px;overflow:visible;position:relative;transition:border-color .2s;width:100%;min-width:280px}.search-box:focus-within{border-color:var(--engine-color);animation:glow-pulse 3s ease-in-out infinite}.search-engine-btn{display:flex;align-items:center;gap:6px;height:44px;padding:0 10px;cursor:pointer;flex-shrink:0;-webkit-tap-highlight-color:transparent}.search-engine-icon{width:32px;height:32px;border-radius:8px;background:var(--engine-color);color:#fff;display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:600;flex-shrink:0}.search-engine-name{font-size:13px;color:#666;white-space:nowrap}.search-engine-arrow{font-size:10px;color:#e0e0e0;line-height:1;transition:color .2s}.search-box:focus-within .search-engine-arrow{color:var(--engine-color)}.search-engine-dropdown{position:absolute;top:48px;left:0;background:#fff;border:1px solid #e0e0e0;border-radius:12px;box-shadow:0 4px 16px rgba(0,0,0,.12);z-index:50;display:none;padding:6px 0;white-space:nowrap}.search-engine-dropdown.show{display:block}.search-engine-option{display:flex;align-items:center;gap:10px;padding:10px 14px 10px 10px;cursor:pointer;-webkit-tap-highlight-color:transparent;font-size:14px}.search-engine-option:active{background:#f0f0f0}.search-engine-option .engine-icon{width:32px;height:32px;border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:16px;font-weight:600;flex-shrink:0}.search-engine-option .engine-name{color:#333}#search-input{flex:1;border:none;background:transparent;padding:14px 8px;font-size:16px;color:#333;outline:none;min-width:0}.search-submit-btn{width:44px;height:44px;display:flex;align-items:center;justify-content:center;cursor:pointer;flex-shrink:0;-webkit-tap-highlight-color:transparent}.search-submit-btn svg{width:20px;height:20px;stroke:#e0e0e0;fill:none;stroke-width:2;transition:stroke .2s}.search-box:focus-within .search-submit-btn svg{stroke:var(--engine-color)}</style><div style="text-align:center"><div class="search-logo" id="search-logo">${this.currentEngine.name}</div></div><div class="search-box" id="search-box"><div class="search-engine-btn" id="engine-btn"><div class="search-engine-icon" id="engine-icon">${this.currentEngine.name.charAt(0)}</div><span class="search-engine-name" id="engine-name">${dn}</span><span class="search-engine-arrow">&#9660;</span></div><div class="search-engine-dropdown" id="engine-dropdown">${this.engines.map(e=>`<div class="search-engine-option" data-engine="${e.name}"><div class="engine-icon" style="background:${e.color}">${e.name.charAt(0)}</div><span class="engine-name">${e.name}</span></div>`).join('')}</div><input type="text" id="search-input" enterkeyhint="go"><div class="search-submit-btn" id="search-submit"><svg viewBox="0 0 24 24"><circle cx="11" cy="11" r="7"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg></div></div>`; },
    ...
    const Shortcuts = {
        ...
        render() {
            ...
            return `<style>#shortcuts-area{position:absolute;top:calc(30% + 180px);left:50%;transform:translateX(-50%);width:80%;display:${c.shortcutsVisible!==false?'grid':'none'};grid-template-columns:repeat(${this.COLS},1fr);grid-template-rows:repeat(${this.ROWS},auto);gap:24px 8px;justify-items:center}.shortcut-item{display:flex;flex-direction:column;align-items:center;gap:10px;cursor:pointer;-webkit-tap-highlight-color:transparent;text-decoration:none;width:72px;position:relative}.shortcut-item.empty{pointer-events:none}.shortcut-item.editing .shortcut-icon{animation:wiggle .3s ease-in-out infinite alternate}@keyframes wiggle{0%{transform:rotate(-1deg)}100%{transform:rotate(1deg)}}.shortcut-item:active .shortcut-icon{transform:scale(.92)}.shortcut-item.dragging{opacity:.4}.shortcut-icon{width:56px;height:56px;border-radius:14px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:26px;font-weight:600;flex-shrink:0;transition:transform .15s;position:relative}.shortcut-delete{position:absolute;top:-6px;right:-6px;width:20px;height:20px;border-radius:50%;background:var(--engine-color,#008373);color:#fff;font-size:11px;display:flex;align-items:center;justify-content:center;cursor:pointer;z-index:2;line-height:1}.placeholder-icon{background:#e0e0e0!important;color:#999!important;font-size:28px}.shortcut-title{font-size:11px;color:#666;text-align:center;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:72px}.shortcut-dialog-overlay{position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.5);z-index:300;display:none;align-items:center;justify-content:center}.shortcut-dialog-overlay.show{display:flex}.shortcut-dialog-box{background:#fff;border-radius:14px;padding:20px;width:80%;max-width:340px;box-shadow:0 8px 30px rgba(0,0,0,.2)}.shortcut-dialog-title{font-size:16px;font-weight:600;color:#333;margin-bottom:16px;text-align:center}.shortcut-dialog-input-wrap{margin-bottom:10px}.shortcut-dialog-input{width:100%;padding:10px 12px;border:1px solid #e0e0e0;border-radius:8px;font-size:14px;color:#333;outline:none;background:#f9f9f9}.shortcut-dialog-input:focus{border-color:var(--engine-color,#008373)}.shortcut-dialog-color-wrap{margin-bottom:10px}.shortcut-dialog-color-label{font-size:12px;color:#999;margin-bottom:6px}.shortcut-dialog-color-pool{display:grid;grid-template-columns:repeat(6,1fr);gap:6px}.shortcut-dialog-color-item{width:100%;aspect-ratio:1;border-radius:6px;cursor:pointer;border:2px solid transparent}.shortcut-dialog-color-item.selected{border-color:#333}.shortcut-dialog-buttons{display:flex;gap:10px;margin-top:6px}.shortcut-dialog-btn{flex:1;padding:10px;border:none;border-radius:8px;font-size:14px;cursor:pointer;-webkit-tap-highlight-color:transparent}.shortcut-dialog-btn-cancel{background:#f0f0f0;color:#666}.shortcut-dialog-btn-confirm{background:var(--engine-color,#008373);color:#fff}</style>${items.join('')}`;
        },
        ...
        init() {
            ...
            if (dialogContainer) {
                dialogContainer.innerHTML = `<div class="shortcut-dialog-overlay" id="shortcut-dialog-overlay"><div class="shortcut-dialog-box"><div class="shortcut-dialog-title" id="shortcut-dialog-title"></div><div class="shortcut-dialog-input-wrap"><input class="shortcut-dialog-input" id="shortcut-dialog-input" placeholder="名称"></div><div class="shortcut-dialog-input-wrap"><input class="shortcut-dialog-input" id="shortcut-dialog-url" placeholder="URL"></div><div class="shortcut-dialog-color-wrap"><div class="shortcut-dialog-color-label">图标颜色</div><div class="shortcut-dialog-color-pool" id="shortcut-dialog-color-pool"></div></div><div class="shortcut-dialog-buttons"><button class="shortcut-dialog-btn shortcut-dialog-btn-cancel" id="shortcut-dialog-cancel">取消</button><button class="shortcut-dialog-btn shortcut-dialog-btn-confirm" id="shortcut-dialog-confirm">确定</button></div></div></div>`;
            }
            this.bindEvents();
            this.initDialog();
        },
    ...
    const Bookmarks = {
        ...
        buildTreeNode(node, depth) { const isF = node.type === 'folder', isE = node.expanded !== false, ind = depth * 20; const color = node.color || Utils.randomColor(); const isRoot = node.id === 'root'; if (isF) { const sc = node.children ? [...node.children] : []; sc.sort((a, b) => a.type === b.type ? 0 : a.type === 'folder' ? -1 : 1); const cHTML = sc.length > 0 ? sc.map(c => this.buildTreeNode(c, depth + 1)).join('') : `<div class="tree-empty" style="padding-left:${ind + 32}px">空</div>`; return `<div class="tree-node" data-id="${node.id}" data-type="folder" ${isRoot ? '' : 'draggable="true"'}><div class="tree-row folder" style="padding-left:${ind}px" data-id="${node.id}"><span class="tree-arrow ${isRoot ? 'hidden' : ''} ${isE ? 'expanded' : ''}" data-id="${node.id}">▶</span><span class="tree-icon">📁</span><span class="tree-title">${node.title}</span><span class="tree-menu-btn" data-id="${node.id}">⋮</span></div><div class="tree-children ${isE ? '' : 'collapsed'}" data-id="${node.id}">${cHTML}</div></div>`; } else { return `<div class="tree-node" data-id="${node.id}" data-type="bookmark" data-url="${node.url || ''}" draggable="true"><div class="tree-row bookmark" style="padding-left:${ind + 32}px"><span class="bm-icon" style="background:${color}">${node.title.charAt(0)}</span><span class="tree-title">${node.title}</span><span class="tree-url">${node.url || ''}</span><span class="tree-menu-btn" data-id="${node.id}">⋮</span></div></div>`; } },
        render() { const root = Config.load().bookmarkRoot; this.sortChildren(root.children); const th = this.buildTreeNode(root, 0); return `<style>#bookmarks-overlay{position:fixed;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:100}#bookmarks-panel{position:fixed;bottom:0;left:0;width:100%;height:70%;background:#fff;border-radius:16px 16px 0 0;box-shadow:0 -4px 20px rgba(0,0,0,.15);z-index:101;transform:translateY(100%);transition:transform .3s ease-out;display:flex;flex-direction:column;pointer-events:auto}#bookmarks-panel.show{transform:translateY(0)}.bookmarks-header{display:flex;align-items:center;justify-content:space-between;padding:16px 20px;border-bottom:1px solid #f0f0f0;flex-shrink:0}.bookmarks-title{font-size:18px;font-weight:600;color:#333}.bookmarks-close{width:32px;height:32px;display:flex;align-items:center;justify-content:center;cursor:pointer;border-radius:50%;-webkit-tap-highlight-color:transparent;font-size:20px;color:#999}.bookmarks-close:active{background:#f0f0f0}.bookmarks-list{flex:1;overflow-y:auto;padding:8px 0;-webkit-overflow-scrolling:touch}.tree-node.dragging{opacity:.4}.tree-row{display:flex;align-items:center;gap:8px;padding:10px 16px;cursor:pointer;-webkit-tap-highlight-color:transparent;font-size:15px;min-height:44px}.tree-row:active{background:#f5f5f5}.tree-arrow{width:20px;height:20px;display:flex;align-items:center;justify-content:center;font-size:10px;color:var(--bm-color,#999);transition:transform .2s,color .2s;flex-shrink:0}.tree-arrow.hidden{visibility:hidden}.tree-arrow.expanded{transform:rotate(90deg)}.tree-icon{width:35px;height:35px;display:flex;align-items:center;justify-content:center;font-size:28px;flex-shrink:0}.bm-icon{width:28px;height:28px;border-radius:6px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:14px;font-weight:600;flex-shrink:0}.tree-title{color:#333;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;flex-shrink:0}.tree-url{font-size:11px;color:#999;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:120px;flex-shrink:1;margin-left:4px}.tree-menu-btn{width:28px;height:28px;display:flex;align-items:center;justify-content:center;cursor:pointer;border-radius:50%;-webkit-tap-highlight-color:transparent;font-size:16px;color:#ccc;flex-shrink:0;letter-spacing:1px;font-weight:700;margin-left:auto}.tree-menu-btn:active{background:#f0f0f0}.context-menu{position:fixed;background:#fff;border:1px solid #e0e0e0;border-radius:10px;box-shadow:0 4px 16px rgba(0,0,0,.15);z-index:200;padding:6px 0;min-width:140px;display:none}.context-menu.show{display:block}.context-menu-item{padding:10px 16px;font-size:14px;color:#333;cursor:pointer;-webkit-tap-highlight-color:transparent}.context-menu-item:active{background:#f0f0f0}.context-menu-item.danger{color:#e55}.context-menu-item.disabled{color:#ccc;pointer-events:none}.tree-children.collapsed{display:none}.tree-empty{padding:8px 16px;color:#ccc;font-size:13px}.dialog-overlay{position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,.5);z-index:300;display:none;align-items:center;justify-content:center}.dialog-overlay.show{display:flex}.dialog-box{background:#fff;border-radius:14px;padding:20px;width:80%;max-width:340px;box-shadow:0 8px 30px rgba(0,0,0,.2)}.dialog-title{font-size:16px;font-weight:600;color:#333;margin-bottom:16px;text-align:center}.dialog-input-wrap{margin-bottom:10px}.dialog-input{width:100%;padding:10px 12px;border:1px solid #e0e0e0;border-radius:8px;font-size:14px;color:#333;outline:none;background:#f9f9f9}.dialog-input:focus{border-color:var(--engine-color,#008373)}.dialog-buttons{display:flex;gap:10px;margin-top:6px}.dialog-btn{flex:1;padding:10px;border:none;border-radius:8px;font-size:14px;cursor:pointer;-webkit-tap-highlight-color:transparent}.dialog-btn-cancel{background:#f0f0f0;color:#666}.dialog-btn-confirm{background:var(--engine-color,#008373);color:#fff}</style><div id="bookmarks-panel"><div class="bookmarks-header"><span class="bookmarks-title">书签</span><span class="bookmarks-close" id="bookmarks-close">✕</span></div><div class="bookmarks-list" id="bookmarks-list">${th}</div></div><div class="context-menu" id="context-menu"></div><div class="dialog-overlay" id="dialog-overlay"><div class="dialog-box"><div class="dialog-title" id="dialog-title"></div><div class="dialog-input-wrap" id="dialog-input-wrap"><input class="dialog-input" id="dialog-input"></div><div class="dialog-input-wrap" id="dialog-url-wrap" style="display:none"><input class="dialog-input" id="dialog-url" placeholder="URL"></div><div class="dialog-buttons"><button class="dialog-btn dialog-btn-cancel" id="dialog-cancel">取消</button><button class="dialog-btn dialog-btn-confirm" id="dialog-confirm">确定</button></div></div></div>`; },
    ...
    const Settings = {
        ...
        render() { const cfg = Config.load(); return `<style>#settings-overlay{position:fixed;top:0;left:0;width:100%;height:100%;pointer-events:none;z-index:100}#settings-panel{position:fixed;bottom:0;left:0;width:100%;height:70%;background:#fff;border-radius:16px 16px 0 0;box-shadow:0 -4px 20px rgba(0,0,0,.15);z-index:101;transform:translateY(100%);transition:transform .3s ease-out;display:flex;flex-direction:column;pointer-events:auto}#settings-panel.show{transform:translateY(0)}.settings-header{display:flex;align-items:center;justify-content:space-between;padding:16px 20px;border-bottom:1px solid #f0f0f0;flex-shrink:0}.settings-title{font-size:18px;font-weight:600;color:#333}.settings-close{width:32px;height:32px;display:flex;align-items:center;justify-content:center;cursor:pointer;border-radius:50%;-webkit-tap-highlight-color:transparent;font-size:20px;color:#999}.settings-close:active{background:#f0f0f0}.settings-content{display:flex;flex:1;overflow:hidden}.settings-tabs{width:100px;flex-shrink:0;overflow-y:auto;border-right:1px solid #f0f0f0;padding:8px 0;-webkit-overflow-scrolling:touch}.settings-tab{padding:14px 12px;font-size:13px;color:#999;cursor:pointer;-webkit-tap-highlight-color:transparent;border-left:2px solid transparent;transition:color .2s,border-color .2s;text-align:center}.settings-tab.active{color:var(--engine-color,#008373);border-left-color:var(--engine-color,#008373);background:#f9f9f9}.settings-body{flex:1;overflow-y:auto;padding:16px;-webkit-overflow-scrolling:touch}.settings-section{display:none}.settings-section.active{display:block}.engine-form{display:flex;align-items:center;gap:8px;margin-bottom:16px;padding-left:12px}.color-picker-wrapper{position:relative;flex-shrink:0}.color-chip{width:32px;height:32px;border-radius:8px;cursor:pointer;-webkit-tap-highlight-color:transparent}.color-pool{position:absolute;top:40px;left:0;background:#fff;border:1px solid #e0e0e0;border-radius:10px;box-shadow:0 4px 16px rgba(0,0,0,.12);padding:6px;display:none;z-index:10;max-height:160px;overflow-y:auto;-webkit-overflow-scrolling:touch;grid-template-columns:repeat(4,1fr);gap:4px}.color-pool.show{display:grid}.color-pool-item{width:32px;height:32px;border-radius:6px;cursor:pointer;-webkit-tap-highlight-color:transparent}.color-pool-item.selected{outline:2px solid #333;outline-offset:1px}.engine-form input{padding:8px 10px;border:1px solid #e0e0e0;border-radius:8px;font-size:13px;color:#333;outline:none;background:#f9f9f9}.engine-form input:focus{border-color:var(--engine-color,#008373)}#eng-name{flex:.6;min-width:50px}#eng-url{flex:2.4;min-width:120px}.engine-form-btn{padding:8px 14px;border:none;border-radius:8px;font-size:13px;cursor:pointer;-webkit-tap-highlight-color:transparent;flex-shrink:0}.btn-add{background:var(--engine-color,#008373);color:#fff}.btn-clear{background:#f0f0f0;color:#666}.engine-list{display:flex;flex-direction:column;gap:8px}.engine-list-item{display:flex;align-items:center;gap:8px;padding:10px 12px;background:#f9f9f9;border-radius:10px;cursor:pointer;-webkit-tap-highlight-color:transparent;border-top:2px solid transparent;border-bottom:2px solid transparent;transition:border-color .15s}.engine-list-item:active{background:#eee}.engine-list-icon{width:32px;height:32px;border-radius:8px;display:flex;align-items:center;justify-content:center;color:#fff;font-size:16px;font-weight:600;flex-shrink:0}.engine-list-info{flex:1;min-width:0}.engine-list-name{font-size:14px;color:#333;font-weight:500}.engine-list-url{font-size:11px;color:#999;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.engine-list-delete{width:28px;height:28px;display:flex;align-items:center;justify-content:center;cursor:pointer;border-radius:50%;-webkit-tap-highlight-color:transparent;font-size:16px;color:#ccc;flex-shrink:0}.engine-list-delete:active{background:#fee;color:#e55}.drag-handle{cursor:grab;color:#ccc;font-size:14px;flex-shrink:0;padding-left:4px}.drag-handle:active{cursor:grabbing}.settings-item{display:flex;align-items:center;justify-content:space-between;padding:14px 0;border-bottom:1px solid #f5f5f5}.settings-label{font-size:15px;color:#333}.settings-desc{font-size:12px;color:#999;margin-top:2px}.toggle-switch{position:relative;display:inline-block;width:48px;height:28px;flex-shrink:0}.toggle-switch input{opacity:0;width:0;height:0}.toggle-slider{position:absolute;cursor:pointer;top:0;left:0;right:0;bottom:0;background:#e0e0e0;border-radius:28px;transition:background .3s}.toggle-slider:before{content:"";position:absolute;height:22px;width:22px;left:3px;bottom:3px;background:#fff;border-radius:50%;transition:transform .3s}.toggle-switch input:checked+.toggle-slider{background:var(--engine-color,#008373)}.toggle-switch input:checked+.toggle-slider:before{transform:translateX(20px)}</style><div id="settings-panel"><div class="settings-header"><span class="settings-title">设置</span><span class="settings-close" id="settings-close">✕</span></div><div class="settings-content"><div class="settings-tabs" id="settings-tabs"><div class="settings-tab active" data-tab="engine">搜索引擎</div><div class="settings-tab" data-tab="appearance">外观设置</div><div class="settings-tab" data-tab="toolbar">工具栏</div><div class="settings-tab" data-tab="webdav">WebDAV</div><div class="settings-tab" data-tab="profile">配置文件</div><div class="settings-tab" data-tab="reset">还原默认</div></div><div class="settings-body"><div class="settings-section active" id="section-engine"><div class="engine-form"><div class="color-picker-wrapper"><div class="color-chip" id="color-chip" style="background:${this.selectedColor}"></div><div class="color-pool" id="color-pool">${ColorPool.map(c => `<div class="color-pool-item ${c === this.selectedColor ? 'selected' : ''}" data-color="${c}" style="background:${c}"></div>`).join('')}</div></div><input type="text" id="eng-name" placeholder="名称"><input type="text" id="eng-url" placeholder="URL"><button class="engine-form-btn btn-add" id="btn-add-engine">添加</button><button class="engine-form-btn btn-clear" id="btn-clear-form">清空</button></div><div class="engine-list" id="engine-list"></div></div><div class="settings-section" id="section-appearance"><div class="settings-item"><div><div class="settings-label">快捷方式模块</div><div class="settings-desc">显示或隐藏主页上的快捷方式图标</div></div><label class="toggle-switch"><input type="checkbox" id="toggle-shortcuts" ${cfg.shortcutsVisible !== false ? 'checked' : ''}><span class="toggle-slider"></span></label></div></div><div class="settings-section" id="section-toolbar"><p style="color:#999;text-align:center;margin-top:40px;">工具栏 — 开发中...</p></div><div class="settings-section" id="section-webdav"><p style="color:#999;text-align:center;margin-top:40px;">WebDAV 同步 — 开发中...</p></div><div class="settings-section" id="section-profile"><p style="color:#999;text-align:center;margin-top:40px;">配置文件导出/导入 — 开发中...</p></div><div class="settings-section" id="section-reset"><p style="color:#999;text-align:center;margin-top:40px;">还原默认设置 — 开发中...</p></div></div></div></div>`; },
    ...
    function renderHomepage() { document.open(); document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,maximum-scale=1,user-scalable=no"><title>主页</title><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:system-ui,-apple-system,sans-serif;background:#fff;color:#333;min-height:100vh;overflow:hidden;-webkit-user-select:none;user-select:none;-webkit-touch-callout:none;touch-action:manipulation}#global-loader{position:fixed;top:16px;right:16px;display:flex;gap:6px;z-index:999;opacity:0;pointer-events:none;transition:opacity .2s}#global-loader.show{opacity:1}.loader-dot{width:8px;height:8px;border-radius:50%;background:var(--loader-color,#008373);animation:dot-bounce .6s ease-in-out infinite}.loader-dot:nth-child(1){animation-delay:0s}.loader-dot:nth-child(2){animation-delay:.15s}.loader-dot:nth-child(3){animation-delay:.3s}@keyframes dot-bounce{0%,100%{transform:translateY(0)}50%{transform:translateY(-8px)}}</style></head><body><div id="global-loader"><div class="loader-dot"></div><div class="loader-dot"></div><div class="loader-dot"></div></div><div id="search-area"></div><div id="shortcuts-area"></div><div id="shortcuts-dialog-container"></div><div id="bookmarks-overlay"></div><div id="settings-overlay"></div></body></html>`); document.close(); document.addEventListener('contextmenu', e => e.preventDefault()); document.addEventListener('gesturestart', e => e.preventDefault()); document.addEventListener('gesturechange', e => e.preventDefault()); document.addEventListener('gestureend', e => e.preventDefault()); Loader.init(); Search.init(); Shortcuts.init(); Bookmarks.init(); Settings.init(); }
